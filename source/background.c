...
int background_functions(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    pvecback[pba->index_bg_cs2_fld] = pba->cs2_fld;
  ...
  /** designer cs2 additions */
  /** TODO add comments  */
  if(pba->has_cs2 == _TRUE_){
    double log10a=log10(a);
    // find  w, dw_dlog10a, int_w_dlog10a depending on spline method
    double cs2, dcs2_dlog10a,d2cs2_dlog10a2, int_cs2_dlog10a;//
    if(pba->cs2_interpolation_method == cs2_linear){
    
        if(pba->recons_wfld == 0) {
          // Find interval that log10(a) lies in
          int cs2_interval_index;
          if (pba->cs2_log10a_vals[pba->cs2_last_index]<=log10a) {
              while(pba->cs2_log10a_vals[pba->cs2_last_index+1]<log10a){
                                    pba->cs2_last_index++;
              }
          }
          else{
              while(pba->cs2_log10a_vals[pba->cs2_last_index]>log10a){
                  pba->cs2_last_index--;
              }
          }
          cs2_interval_index=pba->cs2_last_index;

          double Delta_log10a = log10a - pba->cs2_log10a_vals[cs2_interval_index];
          dcs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a];
          double cs2_i = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2];
          cs2 = cs2_i + dcs2_dlog10a*Delta_log10a;
          int_cs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] + (Delta_log10a)*(cs2_i+dcs2_dlog10a*Delta_log10a/2.0);
          d2cs2_dlog10a2= 0;
        }
        else {
          // Find interval that log10(a) lies in
          ...
          dcs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a];
          double cs2_i = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2];
          cs2 = pow(tanh(pvecback[pba->index_bg_w_fld]),2.) * ( (cs2_i + dcs2_dlog10a*Delta_log10a) + 1.);
          class_test((cs2>1)||(cs2<0),pba->error_message,"cs2 = %g and breaks cs2> 0 or cs2<=1 assumption.", cs2);
          int_cs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] + (Delta_log10a)*(cs2_i+dcs2_dlog10a*Delta_log10a/2.0);
          d2cs2_dlog10a2= 0;
          }
    }
    ...
    if(pba->cs2_interpolation_method == cs2_cubic){
       ...
       // where h = cs2_log10a_vals[i+1]- cs2_log10a_vals[i] is the unnormalized interval length
       // this sets cs2_i(0)=cs21, cs2_i(1)=cs22, cs2_i''(t)= h^2 * d2cs2_by_dlog10a2
       double h = pba->cs2_log10a_vals[cs2_interval_index+1] - pba->cs2_log10a_vals[cs2_interval_index];            // interval size
       double t = (log10a - pba->cs2_log10a_vals[cs2_interval_index])/h;                           // normalized distance from left side of interval
       double r = 1-t;                                                            // normalized distance from right side of interval
       double cs21=pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on left boundary
       double cs22=pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on right boundary
       double ddcs21=h*h*pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_d2cs2_by_dlog10a2];   // cs2''(t) on left boundary
       double ddcs22=h*h*pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_d2cs2_by_dlog10a2]; // cs2''(t) on right boundary

       cs2 = r*cs21 + t*cs22 + ((r*r-1)*r*ddcs21  +(t*t-1)*t*ddcs22)/6.;
       dcs2_dlog10a = ((cs22-cs21)+ ((3*t*t-1)*ddcs22-(3*r*r-1)*ddcs21)/6)/h;
       d2cs2_dlog10a2 = (ddcs22*t+ddcs21*r)/h/h;
       int_cs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
     ...
     if(pba->cs2_interpolation_method == cs2_cubic_arctanh){
       // Find interval that log10(a) lies in
       int cs2_interval_index;
       if(pba->cs2_array[pba->cs2_last_index*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]<=log10a) {
         while(pba->cs2_array[(pba->cs2_last_index+1)*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]<=log10a){
           pba->cs2_last_index++;
         }
       }

       else{
         while(pba->cs2_array[pba->cs2_last_index*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]>log10a){
         pba->cs2_last_index--;
         }
       }
       cs2_interval_index=pba->cs2_last_index;

       // Use the atanh(cs2) spline to calculate cs2(a).
       // For simplicity we normalize the interval we are splining over to [0,1]
       // by mapping  log10a to t = (log10a - cs2_log10a_vals[i])/h
       // where h is the origninal interval length.
       // Thus P(t) = athanh(cs2(log10a)) = t*atanhcs22 + (1-t)*atanhcs21 + ((1-t)*((1-t)^2-1)*ddatanhcs21 + t*(t^2-1)*ddatanhcs22)/6
       //      cs2(t) = tanh(P(t))
       //      dcs2/dlog10a = sech(P(t))^2*P'(t)/h
       //      P'(t) = (atanhcs22-atanhcs21)+(ddatanhcs22-ddatanhcs21)/6 +(t^2*ddatanhcs21-(1-t)^2*ddatanhcs22)
       // The derivative wrt log10a is similarly evaluated Via
       // dcs2_dlog10a = sech()
       // The intergral of cs2 uses information from the atanh spline to
       // evaluate cs2 and d2cs2/dlog10a2 on interval ends.
       // This information is used to approximate cs2(log10a) as a cubic in log10a
       // which gives an integrable approximation for the interpolations.
       //     cs2_i(0)=cs21
       //     cs2_i(1)=cs22
       //     cs2_i''(t)= h^2 * d2cs2_by_dlog10a2
       //     d2cs2_by_dlog10a2 = (1-cs2^2)*(d2atanh_cs2_by_dlog10a2 - 2*cs2*datanh_cs2_by_dlog10a^2)
       double h = pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]
           - pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2_log10a_super];            // interval size
       double t = (log10a - pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2_log10a_super])/h;                           // normalized distance from left side of interval
       double r = 1-t;
       double atanhcs21 = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2_atanh];  // arctanh(cs2) on left boundary
       double atanhcs22 = pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_cs2_atanh];  // arctanh(cs2) on right boundary
       double ddatanhcs21 = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2];  // d^2arctanh(cs2)/dlog10a^2 on left boundary
       double ddatanhcs22 = pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2];  // d^2arctanh(cs2)/dlog10a^2on right boundary
       double cs21= pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on left boundary
       double cs22= pba->cs2_array[(cs2_interval_index+1)*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on right boundary
       double ddcs21=h*h*(1-cs21*cs21)*(ddatanhcs21 - 2*cs21*pow(atanhcs22-atanhcs21-1/6.0*(2*ddatanhcs21+ddatanhcs22),2)); // cs2''(t) on left boundary
       double ddcs22=h*h*(1-cs22*cs22)*(ddatanhcs22 - 2*cs22*pow(atanhcs22-atanhcs21+1/6.0*(ddatanhcs21+2*ddatanhcs22),2)); // cs2''(t) on right boundary

       double atanhcs2 = r*atanhcs21 + t*atanhcs22 + ((r*r-1)*r*ddatanhcs21  +(t*t-1)*t*ddatanhcs22)/6.;
       double datanhcs2 = atanhcs22 - atanhcs21 + (ddatanhcs22*(3.*t*t-1.) - ddatanhcs21*(3*r*r-1.))/6.;
       double ddatanhcs2 = ddatanhcs22*t + ddatanhcs21*r;
       cs2 = tanh(atanhcs2);
       dcs2_dlog10a = (1-cs2*cs2)*datanhcs2/h;
       d2cs2_dlog10a2 =  (1-cs2*cs2)*(ddatanhcs2-2*cs2*datanhcs2*datanhcs2)/h/h;
       int_cs2_dlog10a = pba->cs2_array[cs2_interval_index*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
                       +h*(t*t*(12.0*cs22+(t*t-2)*ddcs22)/24.0
                           -(r*r-1)*(12.0*cs21+(r*r-1)*ddcs21)/24.0);
     }  
    pvecback[pba->index_bg_cs2]=cs2;
    pvecback[pba->index_bg_dcs2_over_dlna]= dcs2_dlog10a*log10(_E_);
    pvecback[pba->index_bg_int_cs2_dlna]= int_cs2_dlog10a*log10(_E_);
 }
  /** end designer cs2 additions */
  
  /** wspli_lin additions */
  /** TODO add comments  */
  if(pba->has_wspl == _TRUE_){
    double log10a=log10(a);
    // find  w, dw_dlog10a, int_w_dlog10a depending on spline method
    double w, dw_dlog10a,d2w_dlog10a2, int_w_dlog10a,rho_wspl;//

    // Find interval that log10(a) lies in
    int w_interval_index;
    if (pba->w_log10a_vals[pba->w_last_index]<=log10a) {
      while(pba->w_log10a_vals[pba->w_last_index+1]<log10a){
        pba->w_last_index++;
      }
    }
    else{
      while(pba->w_log10a_vals[pba->w_last_index]>log10a){
        pba->w_last_index--;
      }
    }
    w_interval_index=pba->w_last_index;


    double Delta_log10a = log10a - pba->w_log10a_vals[w_interval_index];
    dw_dlog10a = pba->w_array[w_interval_index*pba->w_array_num_cols+pba->index_dw_by_dlog10a];
    double w_i = pba->w_array[w_interval_index*pba->w_array_num_cols+pba->index_w];
    w = (w_i + dw_dlog10a*Delta_log10a);
    class_test((w>-1./3.),pba->error_message,"w = %g and breaks w<-1./3. assumption.", w);
    int_w_dlog10a = pba->w_array[w_interval_index*pba->w_array_num_cols+pba->index_int_w_dlog10a] + (Delta_log10a)*(w_i+dw_dlog10a*Delta_log10a/2.0);
    d2w_dlog10a2= 0;
    
    pvecback[pba->index_bg_wspl]=w;
    pvecback[pba->index_bg_dwspl_over_dlna]= dw_dlog10a*log10(_E_);
    pvecback[pba->index_bg_int_wspl_dlna]= int_w_dlog10a*log10(_E_);  
    rho_wspl = pba->Omega0_wspl * pow(pba->H0,2) * pow(1/a,3)*exp(-3*log(10)*int_w_dlog10a);
    pvecback[pba->index_bg_rho_wspl] = rho_wspl;
    
    double cs2_wspl = pba->cs2_0 * w + pba->cs2_1;
    pvecback[pba->index_bg_cs2_wspl] = cs2_wspl;
    class_test(((cs2_wspl<0)||(cs2_wspl>1)),pba->error_message,"DE cs2 = %g breaks cs2<0 or cs2>1 assumption.", cs2_wspl);
    
    rho_tot += rho_wspl;
    p_tot += w*rho_wspl;
    dp_dloga += (dw_dlog10a*log10(_E_)-3.0*w*(1+w))*rho_wspl;
  }
  /** wspli_lin additions */
...
int background_free_input(
  ...
  // designer cs2 addtions
  if(pba->cs2_on != 0.0)
    free(pba->cs2_array);
    free(pba->cs2_log10a_vals);
  // end addtions
  
  // wspl Addtions
  if (pba->Omega0_wspl != 0.){
    free(pba->w_array);
    free(pba->w_log10a_vals);
  }
  // End of Additions
  return _SUCCESS_;
}
...
int background_indices(
  ...
  class_define_index(pba->index_bg_cs2_fld,pba->has_fld,index_bg,1);
  ...
  // Designer cs2 Addtions
  /* inxed for cs2    */
  class_define_index(pba->index_bg_cs2,pba->has_cs2,index_bg,1);
  class_define_index(pba->index_bg_dcs2_over_dlna,pba->has_cs2,index_bg,1);
  class_define_index(pba->index_bg_int_cs2_dlna,pba->has_cs2,index_bg,1);
  // end of additions
  
  // wspl Addtions
  /* inxed for wspl    */
  class_define_index(pba->index_bg_wspl,pba->has_wspl,index_bg,1);
  class_define_index(pba->index_bg_dwspl_over_dlna,pba->has_wspl,index_bg,1);
  class_define_index(pba->index_bg_int_wspl_dlna,pba->has_wspl,index_bg,1);
  class_define_index(pba->index_bg_rho_wspl,pba->has_wspl,index_bg,1);
  class_define_index(pba->index_bg_cs2_wspl,pba->has_wspl,index_bg,1);
  // end of additions
  /*    */
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)rho_wspl",pba->has_wspl);
  class_store_columntitle(titles,"(.)w_fld",pba->has_fld);
  class_store_columntitle(titles,"wspl",pba->has_wspl);
  class_store_columntitle(titles,"dwspl_dlna",pba->has_wspl);
  class_store_columntitle(titles,"int_wspl_dlna",pba->has_wspl);
  class_store_columntitle(titles,"cs2_wspl",pba->has_wspl);
  class_store_columntitle(titles,"cs2_fld",pba->has_fld);
  ...
  // designer cs2 additions
  if(pba->has_cs2){
    class_store_columntitle(titles,"cs2_fld",_TRUE_);
    class_store_columntitle(titles,"dcs2_dlna",_TRUE_);
    class_store_columntitle(titles,"int_cs2_dlna",_TRUE_);
  }
  
  // wspl additions
  if(pba->has_cs2){
    class_store_columntitle(titles,"wspl_fld",_TRUE_);
    class_store_columntitle(titles,"dwspl_dlna",_TRUE_);
    class_store_columntitle(titles,"int_wspl_dlna",_TRUE_);
    class_store_columntitle(titles,"rho_wspl",_TRUE_);
  }
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_wspl],pba->has_wspl,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_fld],pba->has_fld,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_wspl],pba->has_wspl,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dwspl_over_dlna],pba->has_wspl,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_int_wspl_dlna],pba->has_wspl,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_cs2_wspl],pba->has_wspl,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_cs2_fld],pba->has_fld,storeidx);
    ...
    // designer cs2 Addtions
    if (pba->has_cs2) {
      class_store_double(dataptr,pvecback[pba->index_bg_cs2],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_dcs2_over_dlna],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_int_cs2_dlna],_TRUE_,storeidx);
    }
    // end of additions
    
    // wspl Addtions
    if (pba->has_cs2) {
      class_store_double(dataptr,pvecback[pba->index_bg_wspl],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_dwspl_over_dlna],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_int_wspl_dlna],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_rho_wspl],_TRUE_,storeidx);
      class_store_double(dataptr,pvecback[pba->index_bg_cs2_wspl],_TRUE_,storeidx);
    }
    // end of additions
  }
