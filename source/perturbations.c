    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_p_fld],ppt->has_source_delta_fld,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_rho_fld],ppt->has_source_delta_fld,storeidx);
          ...
          class_store_double(dataptr,tk[ppt->index_tp_delta_wspl],ppt->has_source_delta_wspl,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_p_wspl],ppt->has_source_delta_wspl,storeidx);
          class_store_double(dataptr,tk[ppt->index_tp_delta_rho_wspl],ppt->has_source_delta_wspl,storeidx);
        }
        if (ppt->has_velocity_transfers == _TRUE_) {
          ...
          class_store_double(dataptr,tk[ppt->index_tp_theta_wspl],ppt->has_source_theta_wspl,storeidx);
...
int perturbations_output_titles(
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_p_fld",pba->has_fld);
      class_store_columntitle(titles,"d_rho_fld",pba->has_fld);
      ...
      class_store_columntitle(titles,"d_wspl",pba->has_wspl);
      class_store_columntitle(titles,"d_p_wspl",pba->has_wspl);
      class_store_columntitle(titles,"d_rho_wspl",pba->has_wspl);
    }
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_wspl",pba->has_wspl);
    }
  }
...
int perturbations_init(
  ...
  if (pba->has_wspl == _TRUE_) {
    ...
    /* check values of wspl at initial time and today. Since 'a' in the code stands for 'a/a_0', its current value is 1 by definition */
    w_fld_0 = pba->w_log10a_vals[pba->w_num_in_knots-1];
    w_fld_ini = pba->w_log10a_vals[0];
    class_test(w_fld_ini >= 0.,
               ppt->error_message,
               "The fluid is meant to be negligible at early time, and unimportant for defining the initial conditions of other species. You are using parameters for which this assumption may break down, since at early times you have w_fld(a--->0) = %e >= 0",w_fld_ini);
...
int perturbations_indices(
  ...
  /* wspl Additions, basically will copy paste w_fld stuff */
  ppt->has_source_delta_wspl = _FALSE_;
  ppt->has_source_theta_wspl = _FALSE_;
  /* End of Additions */
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        // wspl Additions
        if (pba->has_wspl == _TRUE_)
          ppt->has_source_delta_wspl = _TRUE_;
        // End of Additions
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        // wspl Additons
        if (pba->has_wspl == _TRUE_){
          ppt->has_source_theta_wspl = _TRUE_;
        }
        // End of Additions
      }
      ...
      class_define_index(ppt->index_tp_delta_p_fld,  ppt->has_source_delta_fld, index_type,1);
      class_define_index(ppt->index_tp_delta_rho_fld,  ppt->has_source_delta_fld, index_type,1);
      ...
      /* wspl Additions */
      class_define_index(ppt->index_tp_delta_wspl,  ppt->has_source_delta_wspl, index_type,1);
      class_define_index(ppt->index_tp_delta_rho_wspl,  ppt->has_source_delta_wspl, index_type,1);
      class_define_index(ppt->index_tp_delta_p_wspl,  ppt->has_source_delta_wspl, index_type,1);
      class_define_index(ppt->index_tp_theta_wspl,  ppt->has_source_theta_wspl, index_type,1);
      /* wspl Additions */
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* wspl Additons */
      class_store_columntitle(ppt->scalar_titles, "delta_rho_wspl", pba->has_wspl);
      class_store_columntitle(ppt->scalar_titles, "rho_plus_p_theta_wspl", pba->has_wspl);
      class_store_columntitle(ppt->scalar_titles, "delta_p_wspl", pba->has_wspl);
      /* End of Additions */
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    if(pba->has_wspl == _TRUE_){
      if (pba->use_ppf == _FALSE_) {
        class_define_index(ppv->index_pt_delta_wspl,_TRUE_,index_pt,1); /* density of wspl, will be same thing of wfld */
        class_define_index(ppv->index_pt_theta_wspl,_TRUE_,index_pt,1); /* velocity of wspl, will be same thing of wfld */
      }
    else {
      class_define_index(ppv->index_pt_Gamma_wspl,pba->has_wspl,index_pt,1); /* Gamma variable of PPF scheme */
      }
    }
    /* End of Additions */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_wspl == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ppv->y[ppv->index_pt_delta_wspl] =
            ppw->pv->y[ppw->pv->index_pt_delta_wspl];

          ppv->y[ppv->index_pt_theta_wspl] =
            ppw->pv->y[ppw->pv->index_pt_theta_wspl];
        }
        else {
          ppv->y[ppv->index_pt_Gamma_wspl] =
            ppw->pv->y[ppw->pv->index_pt_Gamma_wspl];
        }
      }
      /* End of Additions */
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double w_fld,dw_over_da_fld,integral_fld, cs2_fld;
  double wspl,cs2_wspl, rho_wspl_over_rho_r ; // wspl addition
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (pba->has_cs2 == _TRUE_) {
          cs2_fld = ppw->pvecback[pba->index_bg_cs2];
        }
        else {  
          cs2_fld = ppw->pvecback[pba->index_bg_cs2_fld];
        }

        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);

        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_fld] = - ktau_two/4.*(1.+w_fld)*(4.-3.*cs2_fld)/(4.-6.*w_fld+3.*cs2_fld) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature

          ppw->pv->y[ppw->pv->index_pt_theta_fld] = - k*ktau_three/4.*cs2_fld/(4.-6.*w_fld+3.*cs2_fld) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature
        }
      ...
      if(pba->has_wspl==_TRUE_){
        ...
        wspl = ppw->pvecback[pba->index_bg_wspl];
        cs2_wspl = ppw->pvecback[pba->index_bg_cs2_wspl];
        
        // TODO: Add has_cs2 here
        // CAVEAT: I will assume PPF will work with this, PPF is terrible anyways
        
        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_wspl] = - ktau_two/4.*(1.+wspl)*(4.-3.*cs2_wspl)/(4.-6.*wspl+3.*cs2_wspl) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature

          ppw->pv->y[ppw->pv->index_pt_theta_wspl] = - k*ktau_three/4.*cs2_wspl/(4.-6.*wspl+3.*cs2_wspl) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature
        }
        /* if use_ppf == _TRUE_, y[ppw->pv->index_pt_Gamma_fld] will be automatically set to zero, and this is what we want (although one could probably work out some small nonzero initial conditions: TODO) */
        
      }
      // End of wspl additons
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      // wspl addtions
      if((pba->has_wspl==_TRUE_) && (pba->use_ppf == _FALSE_)){
        ppw->pv->y[ppw->pv->index_pt_delta_wspl] -= 3*(1.+wspl)*a_prime_over_a*alpha;
        ppw->pv->y[ppw->pv->index_pt_theta_wspl] += k*k*alpha;
      }
      // End of wspl additons
...
int perturbations_total_stress_energy(
  ...
  /** wspl addition */
  double cs2_wspl,wspl,ca2_wspl, w_prime_wspl, dwspl_over_da;
  double Gamma_wspl, rho_plus_p_theta_wspl_prime;
  double rho_wspl, p_wspl, rho_wspl_prime, p_wspl_prime;  
  ...
  if (_scalars_) {
    ...
    */
    ...
    if (pba->has_wspl == _TRUE_){
      wspl = ppw->pvecback[pba->index_bg_wspl];
      cs2_wspl = ppw->pvecback[pba->index_bg_cs2_wspl];
      dwspl_over_da = ppw->pvecback[pba->index_bg_dwspl_over_dlna]/a;
      
      w_prime_wspl = dwspl_over_da * a_prime_over_a * a;

      if (pba->use_ppf == _FALSE_) {
        ppw->delta_rho_wspl = ppw->pvecback[pba->index_bg_rho_wspl]*y[ppw->pv->index_pt_delta_wspl];
        ppw->rho_plus_p_theta_wspl = (1.+wspl)*ppw->pvecback[pba->index_bg_rho_wspl]*y[ppw->pv->index_pt_theta_wspl];
        ca2_wspl = wspl - w_prime_wspl / 3. / (1.+wspl) / a_prime_over_a;
        /** We must gauge transform the pressure perturbation from the fluid rest-frame to the gauge we are working in */
        ppw->delta_p_wspl = cs2_wspl * ppw->delta_rho_wspl + (cs2_wspl-ca2_wspl)*(3*a_prime_over_a*ppw->rho_plus_p_theta_wspl/k/k);
      }
      else {
        s2sq = ppw->s_l[2]*ppw->s_l[2];
        c_gamma_k_H_square = pow(pba->c_gamma_over_c_fld*k/a_prime_over_a,2)*cs2_wspl;
        ...
        */
        if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max)
          ...
          Gamma_wspl = y[ppw->pv->index_pt_Gamma_wspl];

        if (ppt->gauge == synchronous){
          alpha = (y[ppw->pv->index_pt_eta]+1.5*a2/k2/s2sq*(ppw->delta_rho+3*a_prime_over_a/k2*ppw->rho_plus_p_theta)-Gamma_wspl)/a_prime_over_a;
        ...
        ppw->S_wspl = ppw->pvecback[pba->index_bg_rho_wspl]*(1.+wspl)*1.5*a2/k2/a_prime_over_a*
        ...
        /** We must now check the stiffness criterion again and set Gamma_prime_wspl accordingly. */
        if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max){
          ppw->Gamma_prime_wspl = 0.;
        }
        else{
          ppw->Gamma_prime_wspl = a_prime_over_a*(ppw->S_wspl/(1.+c_gamma_k_H_square) - (1.+c_gamma_k_H_square)*Gamma_wspl);
        }
        Gamma_prime_plus_a_prime_over_a_Gamma = ppw->Gamma_prime_wspl+a_prime_over_a*Gamma_wspl;
        // delta and theta in both gauges gauge:
        ppw->rho_plus_p_theta_wspl = ppw->pvecback[pba->index_bg_rho_wspl]*(1.+wspl)*ppw->rho_plus_p_theta/ppw->rho_plus_p_tot-
          k2*2./3.*a_prime_over_a/a2/(1+4.5*a2/k2/s2sq*ppw->rho_plus_p_tot)*
          (ppw->S_wspl-Gamma_prime_plus_a_prime_over_a_Gamma/a_prime_over_a);
        ppw->delta_rho_wspl = -2./3.*k2*s2sq/a2*Gamma_wspl-3*a_prime_over_a/k2*ppw->rho_plus_p_theta_wspl;
        ...
        rho_wspl = ppw->pvecback[pba->index_bg_rho_wspl];
        p_wspl = wspl*rho_wspl;
        rho_wspl_prime = -3*a_prime_over_a*(rho_wspl+p_wspl);
        p_wspl_prime = w_prime_wspl*rho_wspl-3*a_prime_over_a*(1+wspl)*p_wspl;
        rho_t = ppw->pvecback[pba->index_bg_rho_tot] - rho_wspl;
        p_t = ppw->pvecback[pba->index_bg_p_tot] - p_wspl;
        rho_t_prime = -3*a_prime_over_a*(rho_t+p_t);
        p_t_prime = ppw->pvecback[pba->index_bg_p_tot_prime]-p_wspl_prime;
        ...
        S = ppw->S_wspl;
        S_prime = -Z_prime/Z*S+1./Z*(rho_wspl_prime+p_wspl_prime)*(theta_t+k2*alpha)+1./Z*(rho_wspl+p_wspl)*(theta_t_prime+k2*alpha_prime);
        /** Analytic derivative of the equation for ppw->rho_plus_p_theta_wspl above. */
        rho_plus_p_theta_wspl_prime = Z_prime*(S-1./(1.+Y)*(S/(1.+1./X)+Gamma_wspl*X)) +
          Z*(S_prime + Y_prime/(1.+Y*Y+2*Y)*(S/(1.+1./X)+Gamma_wspl*X)-
             1./(1.+Y)*(S_prime/(1.+1./X)+S*X_prime/(1.+X*X+2*X)+ppw->Gamma_prime_wspl*X+Gamma_wspl*X_prime))-
          k2*alpha_prime*(rho_wspl+p_wspl)-k2*alpha*(rho_wspl_prime+p_wspl_prime);

        /** We can finally compute the pressure perturbation using the Euler equation for theta_wspl */
        ppw->delta_p_wspl = (rho_plus_p_theta_wspl_prime+4*a_prime_over_a* ppw->rho_plus_p_theta_wspl - (rho_wspl+p_wspl)*metric_euler)/k2;
      }

      ppw->delta_rho += ppw->delta_rho_wspl;
      ppw->rho_plus_p_theta += ppw->rho_plus_p_theta_wspl;
      ppw->delta_p += ppw->delta_p_wspl;

      ppw->rho_plus_p_tot += (1.+wspl)*ppw->pvecback[pba->index_bg_rho_wspl];
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_) {
        ...
        /** We must gauge transform the pressure perturbation from the fluid rest-frame to the gauge we are working in */
        ppw->delta_p_fld = cs2_fld * ppw->delta_rho_fld + (cs2_fld-ca2_fld)*(3*a_prime_over_a*ppw->rho_plus_p_theta_fld/k/k);
      }
      else {
        s2sq = ppw->s_l[2]*ppw->s_l[2];
        c_gamma_k_H_square = pow(pba->c_gamma_over_c_fld*k/a_prime_over_a,2)*cs2_fld;
        /** The equation is too stiff for Runge-Kutta when c_gamma_k_H_square is large.
            Use the asymptotic solution Gamma=Gamma'=0 in that case.
        */
        if (c_gamma_k_H_square > ppr->c_gamma_k_H_square_max)
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_fld == _TRUE_) {
      ...
      _set_source_(ppt->index_tp_delta_p_fld) = ppw->delta_p_fld;
        
      _set_source_(ppt->index_tp_delta_rho_fld) = ppw->delta_rho_fld;
    }
    ...
    // wspl additons
    if (ppt->has_source_delta_wspl == _TRUE_){
      _set_source_(ppt->index_tp_delta_wspl) = ppw->delta_rho_wspl/pvecback[pba->index_bg_rho_wspl]
        + 3.*a_prime_over_a*(1.+pvecback[pba->index_bg_wspl])*theta_over_k2; // N-body gauge correction
        
      _set_source_(ppt->index_tp_delta_p_wspl) = ppw->delta_p_wspl;
        
      _set_source_(ppt->index_tp_delta_rho_wspl) = ppw->delta_rho_wspl;
      
    }
    if (ppt->has_source_theta_wspl == _TRUE_){
      _set_source_(ppt->index_tp_theta_wspl) = ppw->rho_plus_p_theta_wspl/(1.+pvecback[pba->index_bg_wspl])/pvecback[pba->index_bg_rho_wspl]
        + theta_shift; // N-body gauge correction
    }
    // end of  wspl additons
  }
...
int perturbations_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    // wpls additions
    class_store_double(dataptr, ppw->delta_rho_wspl, pba->has_wspl, storeidx);
    class_store_double(dataptr, ppw->rho_plus_p_theta_wspl, pba->has_fld, storeidx);
    class_store_double(dataptr, ppw->delta_p_wspl, pba->has_wspl, storeidx);
    // End of wspl additions
    //fprintf(ppw->perturbations_output_file,"\n");
...
int perturbations_derivs(double tau,
  ...
  // wspl Additions
  double wspl, cs2_wspl, ca2_wspl, dwspl_over_da, w_prime_wspl;
  // End of Additons
  ...
  if (_scalars_) {
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        if (pba->has_cs2 == _TRUE_) {
          cs2 = ppw->pvecback[pba->index_bg_cs2];
        }
        else {  
          cs2 = ppw->pvecback[pba->index_bg_cs2_fld];
        }
    ...
    if (pba->has_wspl == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        /** - ----> factors w, w_prime, adiabatic sound speed ca2 (all three background-related),
            plus actual sound speed in the fluid rest frame cs2 */
        wspl = ppw->pvecback[pba->index_bg_wspl];
        ca2_wspl = wspl - ppw->pvecback[pba->index_bg_dwspl_over_dlna]/3./(1.+wspl);
        cs2_wspl = ppw->pvecback[pba->index_bg_cs2_wspl];
        dwspl_over_da = ppw->pvecback[pba->index_bg_dwspl_over_dlna]/a;
        ...
        dy[pv->index_pt_delta_wspl] =
          -(1+wspl)*(y[pv->index_pt_theta_wspl]+metric_continuity)
          -3.*(cs2-wspl)*a_prime_over_a*y[pv->index_pt_delta_wspl]
          -9.*(1+wspl)*(cs2_wspl-ca2_wspl)*a_prime_over_a*a_prime_over_a*y[pv->index_pt_theta_wspl]/k2;

        /** - ----> fluid velocity */

        dy[pv->index_pt_theta_wspl] = /* fluid velocity */
          -(1.-3.*cs2_wspl)*a_prime_over_a*y[pv->index_pt_theta_wspl]
          +cs2*k2/(1.+wspl)*y[pv->index_pt_delta_wspl]
          +metric_euler;
      }
      else {
        dy[pv->index_pt_Gamma_wspl] = ppw->Gamma_prime_wspl; /* Gamma variable of PPF formalism */
      }
