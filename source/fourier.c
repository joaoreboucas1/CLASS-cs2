  ...
  */
  ...
  if (pfo->has_pk_eq == _FALSE_) {
    ...
    if (pba->has_wspl == _TRUE_) {
      //w = pvecback[pba->index_bg_wspl];
      //dw_over_da_fld = ppw->pvecback[pba->index_bg_dwspl_over_dlna]/a;
      //integral_fld = 0.;
      class_stop(pfo->error_message,"Halofit and wspl not yet implemented. Do like w0-wa");
    }
    else {
...
int fourier_hmcode_dark_energy_correction(
  ...
  if (pba->has_wspl==_TRUE_){
      ...
      class_stop(pba->error_message,"No wspl and hmcode implementation");
