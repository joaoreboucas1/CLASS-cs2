...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_call(parser_read_double(pfc,"Omega_fld",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_wspl",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  //class_call(parser_read_double(pfc,"Omega_wspl",&param4,&flag4,errmsg),
  //           errmsg,
  //           errmsg);
  /* Test */
  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && ((flag3 == _FALSE_) || (param3 >= 0.)),
             errmsg,
             "'Omega_Lambda' or 'Omega_fld' must be left unspecified, except if 'Omega_wspl' is set and < 0.");
  class_test(((flag1 == _FALSE_)||(flag2 == _FALSE_)) && ((flag3 == _TRUE_) && (param3 < 0.)),
             errmsg,
             "You have entered 'Omega_wspl' < 0 , so you have to specify both 'Omega_lambda' and 'Omega_fld'.");
  /* Complete set of parameters
  ...
  if ((flag3 == _TRUE_) && (param3 >= 0.)){
    pba->Omega0_wspl = param3;
    Omega_tot += pba->Omega0_wspl;
  }
  //if (flag4 == _TRUE_){
  //  pba->Omega0_wspl = param4;
  //  Omega_tot += pba->Omega0_wspl;
  //}
  /* Step 2 */
  if (flag1 == _FALSE_) {
    ...
    pba->Omega0_wspl = 1. - pba->Omega0_k - Omega_tot;
    if (input_verbose > 0){
      printf(" -> matched budget equations by adjusting Omega_wspl = %g\n",pba->Omega0_wspl);
    }
  }
  ...
  // Read in w spline anchors in log10a
  if (pba->Omega0_wspl != 0.) {
    /** 8.a.4) PPF approximation */
    /* Read */
    class_call(parser_read_string(pfc,"use_ppf",&string1,&flag1,errmsg),
               errmsg,
               errmsg);
    ...
    class_test(flag1!=flag2,errmsg,"w vals not provided for designer w. Check .ini file.");
    //printf(" pointer[0]=%g \n", pointerw[0]);
    
    // Test to ensure we have the same number of log(a) and delta values
    class_test(pba->w_num_in_knots != n,errmsg,"Number of w independent and dependant values are mismatched. Found %d independent and %d dependant.  Check your .ini file.",pba->w_num_in_knots,n);
    
    
    // Read what redshift the w calibrator alpha is given.
    pba->w_z_alpha = 0.0;
    //class_call(parser_read_double(pfc,"w_z_alpha",&(pba->w_z_alpha),&flag1,errmsg),
    //         errmsg,
    //         errmsg);

    //class_test(pba->w_z_alpha !=0.0 && flag2==_TRUE_,errmsg,"Splined w only accommodates z_alpha with w_z_alpha and not with Omega_wspl. Check your .ini file.");
    
    //Test that log(a) values contain today, and z_alpha and a_ini
    int w_ref_interval_idx=0;
    int w_a_today_interval_idx=0;
    double log10a_alpha=-1.0*log10(pba->w_z_alpha+1.0);
    double log10a_today= 0.0;
    class_test(pba->w_log10a_vals[0]>log10(ppr->a_ini_over_a_today_default),errmsg,"w log10a range does not include a_ini. Check your .ini file.");
    class_test((log10a_alpha>log10a_today)|(log10a_alpha<log10(ppr->a_ini_over_a_today_default)),errmsg,"w z_alpha not contained in a_ini to a_today. Check your .ini file.");
    class_test(pba->w_log10a_vals[pba->w_num_in_knots-1]<log10a_today,errmsg,"w log10a range does not include a_today. Check your .ini file.");
    
    // Stores the integral from a_alpha to a_today for setting Omega0_wspl
    double int_w_alpha_to_today;
    
    /* Begin of linear spline */
     
    // Allocate the spline table and set indices
    pba->index_w = 0;
    pba->index_dw_by_dlog10a = 1;
    pba->index_int_w_dlog10a = 2;
    pba->w_array_num_cols = 3 ;
    class_alloc(pba->w_array,pba->w_array_num_cols*pba->w_num_in_knots*sizeof(double),errmsg);

    // Store w values in the spline array and ensure parameters are between 0 and 1

    for (size_t i = 0; i < pba->w_num_in_knots; i++) {
      pba->w_array[i*pba->w_array_num_cols+pba->index_w]=pointerw[i];
      class_test((pba->w_array[i*pba->w_array_num_cols+pba->index_w]<-1)&&(pba->use_ppf==_FALSE_),errmsg,"Phantom crossing turn on PPF",i,pba->w_array[i*pba->w_array_num_cols+pba->index_w]);
      class_test(pba->w_array[i*pba->w_array_num_cols+pba->index_w]>-1./3.,errmsg,"w value indexed %d read as %g, pointer=%g and breaks w<=-1/3 assumption.",i,pba->w_array[i*pba->w_array_num_cols+pba->index_w], pointerw[i]);
      }
    // Free the storage of w values now that they've been stored in spline array
    free(pointerw);

    // Find interval containing a_today
    for (size_t i = 1; i < pba->w_num_in_knots; i++) {
      if(pba->w_log10a_vals[i]<=log10a_alpha) w_ref_interval_idx++;
      if(pba->w_log10a_vals[i]<=log10a_today) w_a_today_interval_idx++;
        class_test(pba->w_log10a_vals[i-1]>=pba->w_log10a_vals[i],pba->error_message,"w indexed %d and %d are out of chronological order, anchors should be increasing values of Log(a). Check your .ini file.",i-1,i);
    }

    pba->w_array[0*pba->w_array_num_cols+pba->index_int_w_dlog10a]=0;
    // Initialize derivatives and integrals
    for (size_t i = 0; i < pba->w_num_in_knots-1; i++) {
      double Delta_log10a = pba->w_log10a_vals[i+1] - pba->w_log10a_vals[i];
      double Delta_w = pba->w_array[(i+1)*pba->w_array_num_cols+pba->index_w] - pba->w_array[i*pba->w_array_num_cols+pba->index_w];
      double w_i = pba->w_array[i*pba->w_array_num_cols+pba->index_w];
      pba->w_array[i*pba->w_array_num_cols+pba->index_dw_by_dlog10a] = Delta_w/Delta_log10a;
      pba->w_array[(i+1)*pba->w_array_num_cols+pba->index_int_w_dlog10a] =  pba->w_array[i*pba->w_array_num_cols+pba->index_int_w_dlog10a] + Delta_log10a*(w_i+Delta_w/2.0);
    }
    // Last point continues previous derivative
    pba->w_array[(pba->w_num_in_knots-1)*pba->w_array_num_cols+pba->index_dw_by_dlog10a] = pba->w_array[(pba->w_num_in_knots-2)*pba->w_array_num_cols+pba->index_dw_by_dlog10a];

    // Shift integrals for where rho_wspl information is provided
    // Integrate out from log10a_alpha to w_log10a_vals[i]
    // Shift stored integrals so the new lower bound is log10a_alpha
      
    double Delta_log10a_ref = (log10a_alpha-pba->w_log10a_vals[w_ref_interval_idx]);
    double Delta_w_ref = Delta_log10a_ref * pba->w_array[w_ref_interval_idx*pba->w_array_num_cols+pba->index_dw_by_dlog10a];
    double w_ref = pba->w_array[w_ref_interval_idx*pba->w_array_num_cols+pba->index_w];
    double w_integral_ref = pba->w_array[w_ref_interval_idx*pba->w_array_num_cols+pba->index_int_w_dlog10a] + Delta_log10a_ref * (w_ref+Delta_w_ref/2.0);
    for (size_t i = 0; i < pba->w_num_in_knots; i++){
      pba->w_array[i*pba->w_array_num_cols+pba->index_int_w_dlog10a] -=  w_integral_ref;
    }
      
    // Calculate the integral of w from the
    double Delta_log10a = log10a_today-pba->w_log10a_vals[w_a_today_interval_idx];
    double Delta_w = Delta_log10a * pba->w_array[w_a_today_interval_idx*pba->w_array_num_cols+pba->index_dw_by_dlog10a];
    double w_i = pba->w_array[w_a_today_interval_idx*pba->w_array_num_cols+pba->index_w];
    int_w_alpha_to_today = pba->w_array[w_a_today_interval_idx*pba->w_array_num_cols+pba->index_int_w_dlog10a] + Delta_log10a * (w_i+Delta_w/2.0);
      
      
    /* FUTURO
    // Read in the fractional comparison rho_wspl/(rho_rad+rho_m) at z_alpha
    class_call(parser_read_double(pfc,
                                  "wspl_alpha",
                                  &param1,
                                  &flag1,
                                  errmsg),
               errmsg,errmsg);
    class_test(!flag1^(pba->Omega0_gdm!=0),errmsg,"Splined w exclusively requires omega_wspl or wspl_alpha where alpha=rho_wspl/rho_bg at z=z_apha. Check your .ini file.");
    // Calculate Omega0_wspl = rho_wspl_0/ H0^2
    // Where rho_wspl_0= rho_alpha * (z_alpha+1)^-3 exp(-3(int w frop lna_alpha to lna_today ))
    // we must also include a factor of log(10) to account for our integral uses log10a
    if(flag1==_TRUE_){
      // Calculate rho_wspl(z_alpha)
      pba->rho_alpha_wspl = param1
                           *((pba->Omega0_g+pba->Omega0_ur)*(pba->gdm_z_alpha+1)+(pba->Omega0_b+pba->Omega0_cdm))
                           *pow(pba->w_z_alpha+1,3)
                           *pba->H0*pba->H0;
      // Evolve rho_alpha to a_today to set Omega0_wspl
      pba->Omega0_wspl = pba->rho_alpha_wspl
                        *pow(1/(pba->wspl_z_alpha+1),3)
                        *exp(-3.0*log(10)*int_w_alpha_to_today)/pba->H0/pba->H0;
    }
    else{
      pba->rho_alpha_wspl = pba->Omega0_wspl*pba->H0*pba->H0;
    } */
  ...
  /* Begin dsg_cs2_additions */
  
  //Turning on cs2_on hardcoded!!
  // I added cs2_on to turn on dsg, similar to Omega0_fld != 0 and _scf.
  // Default value is 0.
  //if(pba->cs2_on != 0.){

  // Read in dsg cs2 spline anchors in log10a
    class_call(parser_read_list_of_doubles(pfc,
                                         "cs2log10a_vals",
                                         &(pba->cs2_num_in_knots),
                                         &(pba->cs2_log10a_vals),
                                         &flag1,
                                         errmsg),
               errmsg,errmsg);
    ...
    if(flag1==_TRUE_){    
      ...
      // Read in cs2 spline values for cs2
      double* pointercs2;
      class_call(parser_read_list_of_doubles(pfc,
                                           "cs2_vals",
                                           &(n),
      ...
      // Test to ensure both log10(a) and cs2 values provided
      class_test(flag1!=flag2,errmsg,"cs2 vals not provided for designer cs2. Check .ini file.");
      printf(" pointer[0]=%g \n", pointercs2[0]);
    
      // Test to ensure we have the same number of log(a) and delta values
      class_test(pba->cs2_num_in_knots != n,errmsg,"Number of cs2 independent and dependant values are mismatched. Found %d independent and %d dependant.  Check your .ini file.",pba->cs2_num_in_knots,n);
    
      // Read what redshift the cs2 calibrator alpha is given.
      pba->cs2_z_alpha = 0.0;
      class_call(parser_read_double(pfc,"cs2_z_alpha",&(pba->cs2_z_alpha),&flag1,errmsg),
               errmsg,
               errmsg);

      int cs2_order;
      class_call(parser_read_int(pfc,"cs2_interpolation_order",&cs2_order,&flag1,errmsg),
               errmsg,
               errmsg);
      // Assumes cubic unless specified
      pba->cs2_interpolation_method = cs2_cubic;

      if(flag1 == _TRUE_){
          switch(cs2_order){
              case 1: pba->cs2_interpolation_method = cs2_linear; break;
              case 3: pba->cs2_interpolation_method = cs2_cubic; break;
              default: class_test(_FALSE_,errmsg,"Interpolation order not supported, must be 1 or 3"); break;
          }
      }


      if(pba->cs2_interpolation_method== cs2_cubic){
          class_call(parser_read_string(pfc,"cs2_spline_regulator",&string1,&flag1,errmsg),errmsg,errmsg);
          if(flag1==_TRUE_ && strstr(string1,"tanh") != NULL){
              pba->cs2_interpolation_method = cs2_cubic_arctanh;
          }
      }
      //Test that log(a) values contain today, and z_alpha and a_ini
      int cs2_ref_interval_idx=0;
      int cs2_a_today_interval_idx=0;
      double log10a_alpha=-1.0*log10(pba->cs2_z_alpha+1.0);
      double log10a_today= 0.0;
      class_test(pba->cs2_log10a_vals[0]>log10(ppr->a_ini_over_a_today_default),errmsg,"cs2 log10a range does not include a_ini. Check your .ini file.");
      class_test((log10a_alpha>log10a_today)|(log10a_alpha<log10(ppr->a_ini_over_a_today_default)),errmsg,"cs2 z_alpha not contained in a_ini to a_today. Check your .ini file.");
      class_test(pba->cs2_log10a_vals[pba->cs2_num_in_knots-1]<log10a_today,errmsg,"cs2 log10a range does not include a_today. Check your .ini file.");
    
      // Stores the integral from a_alpha to a_today
      double int_cs2_alpha_to_today;
      
      class_read_int("recons_wfld",pba->recons_wfld);
    
      if(pba->cs2_interpolation_method == cs2_linear){

            // Allocate the spline table and set indices
            pba->index_cs2 = 0;
            pba->index_dcs2_by_dlog10a = 1;
            pba->index_int_cs2_dlog10a = 2;
            pba->cs2_array_num_cols = 3 ;
            class_alloc(pba->cs2_array,pba->cs2_array_num_cols*pba->cs2_num_in_knots*sizeof(double),errmsg);
            
            // DO RECONS ON TOP OF WLFD!
            if(pba->recons_wfld != 0) {
              // Store cs2 values in the spline array and ensure parameters are between 0 and 1

              for (size_t i = 0; i < pba->cs2_num_in_knots; i++) {
                  pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]=pointercs2[i];
                  //class_test((pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]>1)||(pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]<0),errmsg,"cs2 value indexed %d read as %g, pointer=%g and breaks cs2> 0 or cs2<=1 assumption.",i,pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2], pointercs2[i]);
              }
              // Free the storage of cs2 values now that they've been stored in spline array
              free(pointercs2);

              // Find interval containing a_today
              for (size_t i = 1; i < pba->cs2_num_in_knots; i++) {
                  if(pba->cs2_log10a_vals[i]<=log10a_alpha) cs2_ref_interval_idx++;
                  if(pba->cs2_log10a_vals[i]<=log10a_today) cs2_a_today_interval_idx++;
                  class_test(pba->cs2_log10a_vals[i-1]>=pba->cs2_log10a_vals[i],pba->error_message,"cs2 indexed %d and %d are out of chronological order, anchors should be increasing values of Log(a). Check your .ini file.",i-1,i);
              }

              pba->cs2_array[0*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]=0;
              // Initialize derivatives and integrals
              for (size_t i = 0; i < pba->cs2_num_in_knots-1; i++) {
                  double Delta_log10a = pba->cs2_log10a_vals[i+1] - pba->cs2_log10a_vals[i];
                  double Delta_cs2 = pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2] - pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2];
                  double cs2_i = pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2];
                  pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a] = Delta_cs2/Delta_log10a;
                  pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] =  pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] + Delta_log10a*(cs2_i+Delta_cs2/2.0);
              }
              // Last point continues previous derivative
              pba->cs2_array[(pba->cs2_num_in_knots-1)*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a] = pba->cs2_array[(pba->cs2_num_in_knots-2)*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a];

              // Shift integrals for where rho_gdm information is provided
              // Integrate out from log10a_alpha to gdm_log10a_vals[i]
              // Shift stored integrals so the new lower bound is log10a_alpha
              {
                  double Delta_log10a_ref = (log10a_alpha-pba->cs2_log10a_vals[cs2_ref_interval_idx]);
                  double Delta_cs2_ref = Delta_log10a_ref * pba->cs2_array[cs2_ref_interval_idx*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a];
                  double cs2_ref = pba->cs2_array[cs2_ref_interval_idx*pba->cs2_array_num_cols+pba->index_cs2];
                  double cs2_integral_ref = pba->cs2_array[cs2_ref_interval_idx*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] + Delta_log10a_ref * (cs2_ref+Delta_cs2_ref/2.0);
                  for (size_t i = 0; i < pba->cs2_num_in_knots; i++){
                      pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] -=  cs2_integral_ref;
                  }
              }
              // Calculate the integral of w from the
              double Delta_log10a = log10a_today-pba->cs2_log10a_vals[cs2_a_today_interval_idx];
              double Delta_cs2 = Delta_log10a * pba->cs2_array[cs2_a_today_interval_idx*pba->cs2_array_num_cols+pba->index_dcs2_by_dlog10a];
              double cs2_i = pba->cs2_array[cs2_a_today_interval_idx*pba->cs2_array_num_cols+pba->index_cs2];
              int_cs2_alpha_to_today = pba->cs2_array[cs2_a_today_interval_idx*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] + Delta_log10a * (cs2_i+Delta_cs2/2.0);
            }
      ...
      if(pba->cs2_interpolation_method == cs2_cubic){
        ...
        for (size_t i = 0; i < pba->cs2_num_in_knots; i++) {
            pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]=pointercs2[i];
            class_test((pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]>1)||(pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]<0),errmsg,"cs2 value indexed %d read as %g and breaks cs2>0 or cs2<=1 assumption. Check your .ini file.",i,pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2]);
        }
        // Free the storage of cs2 values now that they've been stored in spline array
        free(pointercs2);
        ...
        for (size_t i = 1; i < pba->cs2_num_in_knots; i++) {
          if(pba->cs2_log10a_vals[i]<=log10a_alpha) cs2_ref_interval_idx++;
          if(pba->cs2_log10a_vals[i]<=log10a_today) cs2_a_today_interval_idx++;
          class_test(pba->cs2_log10a_vals[i-1]>=pba->cs2_log10a_vals[i],pba->error_message,"cs2 anchors indexed %d and %d are out of cronological order, anchors should be increasing values of Log(a). Check your .ini file.",i-1,i);
        }

        // Initialize spline
        class_call(array_spline_table_line_to_line(pba->cs2_log10a_vals,
                                                 pba->cs2_num_in_knots,
                                                 pba->cs2_array,
                                                 pba->cs2_array_num_cols,
                                                 pba->index_cs2,
                                                 pba->index_d2cs2_by_dlog10a2,
                                                 _SPLINE_NATURAL_,
                                                 pba->error_message),
                 pba->error_message,
                 pba->error_message);
        ...
        // Integrate out from log10a_alpha to gdm_log10a_vals[i]
        // Do check to ensure cs2<=1
        // Store value of integral from log10a_alpha
        double h,cs21,cs22,ddcs21,ddcs22,t,r,cs2_ext,desc,desc_sqrt,wrksp_intgrl;
        size_t spline_case; // tracks if spline is cubic or a degenerate case
        // Initialize integral to 0 at cs2_log10a_vals[0]
        ...
        for(size_t i=0;i<pba->cs2_num_in_knots-1;i++){
          ...
          // normalize such that the original interval of log10a in [cs2_log10a_vals[i],cs2_log10a_vals[i+1]]
          // is linearly mapped to t in [0,1] via t = (log10a - cs2_log10a_vals[i])/h
          // where h = cs2_log10a_vals[i+1]- cs2_log10a_vals[i] is the interval length
          // this sets cs2_i(0)=cs21, cs2_i(1)=cs22,  cs2_i''(t)= h^2 * d2cs2_by_dlog10a2
          h = pba->cs2_log10a_vals[i+1] - pba->cs2_log10a_vals[i];            // interval size
          cs21=pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on left boundary
          cs22=pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2];  // cs2 on right boundary
          ddcs21=h*h*pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_d2cs2_by_dlog10a2];   // cs2''(t) on left boundary
          ddcs22=h*h*pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_d2cs2_by_dlog10a2]; // cs2''(t) on right boundary

          // Integrate over the interval recored the accumulated integral from inital
          pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
            =pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
              + h*(12*(cs21+cs22)-(ddcs21+ddcs22))/24.0;
          // Check to ensure 0<cs2<1
          // We previously explicitly checked 0<cs2<1 holds for the spline knots.
          // Thus only way for 0<cs2<1 to fail on a spline section is if a extremum
          ...
          //         Requires ddcs21==ddcs22
          //         Extremum at t = 1/2 + (cs21-w2)/ddcs21
          // Case #3 Spline section is cubic:
          //         Extremum at (ddcs21 +- sqrt(descrim))/(ddcs21-ddcs22)
          //         Where descrim = (ddcs21+ddcs22)^2- ddcs21*ddcs22 -6(ddcs22-ddcs21)(cs22-cs21)
          spline_case=3;
          if(ddcs21==ddcs22){
            spline_case--;
            if(ddcs21==0) spline_case--;
          }
          switch(spline_case){
            case 1:   // Trivial pass
            case 2:{
              t = (1.0/2.0 +(cs21-cs22)/(ddcs21)); // extremum distance from right side of interval
               if(t>0 && t<1){
                 r = 1-t; // distance from right side of interval
                 // calculate w(t_extremum) simplified assuming ddcs21==ddcs22
                 cs2_ext = r*cs21 + t*cs22 + ((r*r-1)*r*ddcs21  +(t*t-1)*t*ddcs22)/6.;
                 class_test(cs2_ext>1 || cs2_ext <0,
                            errmsg,
                            "0<cs2<=1 in interval indexed %i, check cs2_vals",i);
               }
            }
            case 3:{
              desc = (pow(ddcs21+ddcs22,2)
                      -ddcs21*ddcs22
                      -6*(ddcs22-ddcs21)*(cs22 - cs21)
                     )/3.0;
              // check if extrema is in real domain
              if(desc>=0){
                desc_sqrt = sqrt(desc);
                t = (ddcs21-desc_sqrt)/(ddcs21-ddcs22);
                if(t>0 && t<1 ){
                  r = 1-t; // distance from right side of interval
                  cs2_ext = r*cs21 + t*cs22 + ((r*r-1)*r*ddcs21  +(t*t-1)*t*ddcs22)/6.;
                  //printf("(t=%e, cs21=%e, cs22=%e, ddcs21=%e, ddcs22=%e, cs2_ex=%e)\n",t,cs21,cs22,ddcs21,ddcs22,cs2_ext);
                  class_test(cs2_ext > 1 || cs2_ext < 0,
                             errmsg,
                             "0<cs2<=1 in interval indexed %i, check cs2_vals",i);
                }
          ...
          // Calculate int(cs2) on interval [cs2_log10a_vals[0],log10a_alpha]
          if(i ==  cs2_ref_interval_idx){
            t = (log10a_alpha- pba->cs2_log10a_vals[i])/h;
            r = 1-t;
            wrksp_intgrl=pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
                         +h*(t*t*(12.0*cs22+(t*t-2)*ddcs22)/24.0
                             -(r*r-1)*(12.0*cs21+(r*r-1)*ddcs21)/24.0);
            ...
            for(size_t j=0; j<i+2;j++){
              pba->cs2_array[j*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a] -= wrksp_intgrl;
            }
          }
          if(i ==  cs2_a_today_interval_idx){
            t = (log10a_today - pba->cs2_log10a_vals[i])/h;
            r = 1-t;
            // store the intergral from a_alpha to a_today, since a_alpha <= a_today the integral 0 shift will have already occured
            int_cs2_alpha_to_today=pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
                                 +h*(t*t*(12.0*cs22+(t*t-2)*ddcs22)
                                     -(r*r-1)*(12.0*cs21+(r*r-1)*ddcs21))/24.0;
      ...
      if(pba->cs2_interpolation_method == cs2_cubic_arctanh){
        double cs2_max_spl_Delta_log10a = 0.1;
        class_call(parser_read_double(pfc,"cs2_max_spl_delta_log10a",&cs2_max_spl_Delta_log10a,&flag1,errmsg),
                   errmsg ,
                   errmsg);
        int cs2_num_super_sample_knots = 0;
        int* cs2_super_sample_knot_group_size;
        class_alloc(cs2_super_sample_knot_group_size ,pba->cs2_num_in_knots*sizeof(int),errmsg);
        for(int knot_idx; knot_idx < pba->cs2_num_in_knots-1; knot_idx++){
          cs2_super_sample_knot_group_size[knot_idx] = (int) (ceil((pba->cs2_log10a_vals[knot_idx+1]-pba->cs2_log10a_vals[knot_idx])/cs2_max_spl_Delta_log10a));
          cs2_num_super_sample_knots += cs2_super_sample_knot_group_size[knot_idx];
        }
        cs2_num_super_sample_knots++;

        pba->cs2_num_super_sample_knots = cs2_num_super_sample_knots;

        pba->index_cs2_log10a_super = 0;
        pba->index_cs2 = 1;
        pba->index_cs2_atanh =2;
        pba->index_d2atanh_cs2_by_dlog10a2 = 3;
        pba->index_int_cs2_dlog10a = 4;
        pba->cs2_array_num_cols = 5;

        // Initialize cs2 array
        class_alloc(pba->cs2_array,pba->cs2_array_num_cols*cs2_num_super_sample_knots*sizeof(double),errmsg);

        // initialize indices for a_today and a_alpha

        short found_a_alpha=0, found_a_today=0;

        // Store cs2 values in the spline array and ensure parameters are between 0 and 1
        // Also find the interval for a_alpha and a_today
        int cs2_super_sample_idx = 0;
        double cs2_log10a_diff, cs2_diff, cs2_wrk_log10a, cs2_wrk, cs2_step;
        for (size_t input_knot_idx = 0; input_knot_idx < pba->cs2_num_in_knots-1; input_knot_idx++) {
            cs2_wrk_log10a = pba->cs2_log10a_vals[input_knot_idx];
            cs2_log10a_diff = pba->cs2_log10a_vals[input_knot_idx+1]-pba->cs2_log10a_vals[input_knot_idx];
            class_test(cs2_log10a_diff<=0,errmsg,"cs2 log10a values are not strictly ascending at index %d. Check your .ini file.", input_knot_idx);
            cs2_wrk = pointercs2[input_knot_idx];
            cs2_diff = pointercs2[input_knot_idx+1]-pointercs2[input_knot_idx];
            class_test((cs2_wrk>1)||(cs2_wrk<0),errmsg,"cs2 value indexed %d read as %g and breaks 0<cs2<=1 assumption. Check your .ini file.",input_knot_idx,pointercs2[input_knot_idx]);
            cs2_step = cs2_diff*cs2_max_spl_Delta_log10a/cs2_log10a_diff;
            ...
            for(int sub_group_idx=0; sub_group_idx<cs2_super_sample_knot_group_size[input_knot_idx]; sub_group_idx++){
              pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]=cs2_wrk_log10a;
              pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2]=cs2_wrk;
              pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2_atanh]=atanh(cs2_wrk);
              cs2_wrk_log10a += cs2_max_spl_Delta_log10a;
              if(!found_a_alpha && cs2_wrk_log10a >log10a_alpha){
                found_a_alpha=1;
                cs2_ref_interval_idx=cs2_super_sample_idx;
              };
              if(!found_a_today && cs2_wrk_log10a >log10a_today){
                found_a_today=1;
                cs2_a_today_interval_idx=cs2_super_sample_idx;
              };
              cs2_wrk += cs2_step;
              cs2_super_sample_idx++;
            }
        }
        free(cs2_super_sample_knot_group_size);
        // Store last point
        class_test((pointercs2[pba->cs2_num_in_knots-1]>1)||(pointercs2[pba->cs2_num_in_knots-1]<0),errmsg,"cs2 value indexed %d read as %g and breaks 0<cs2<=1 assumption. Check your .ini file.",pba->cs2_num_in_knots-1,pointercs2[pba->cs2_num_in_knots-1]);
        ...
        if(!found_a_alpha){
          ...
          cs2_ref_interval_idx=cs2_super_sample_idx-1;
        }
        if(!found_a_today){
            found_a_today=_TRUE_;
            cs2_a_today_interval_idx=cs2_super_sample_idx-1;
          }
        pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]=pba->cs2_log10a_vals[pba->cs2_num_in_knots-1];
        pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2]=pointercs2[pba->cs2_num_in_knots-1];
        pba->cs2_array[cs2_super_sample_idx*pba->cs2_array_num_cols+pba->index_cs2_atanh]=atanh(pointercs2[pba->cs2_num_in_knots-1]);

        // Free the storage of cs2 values now that they've been stored in spline array
        free(pointercs2);

        array_spline(pba->cs2_array,
          		       pba->cs2_array_num_cols,
                     pba->cs2_num_super_sample_knots,
                     pba->index_cs2_log10a_super,
                     pba->index_cs2_atanh,
                     pba->index_d2atanh_cs2_by_dlog10a2,
                    _SPLINE_NATURAL_,
                     errmsg);

        // Integrate out from log10a_alpha to cs2_log10a_vals[i]
        // Store value of integral from log10a_alpha
        double h,cs21,cs22,ddcs21,ddcs22,t,r,cs2_ext,wrksp_intgrl;
        ...
        pba->cs2_array[0*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]=0;
        for(size_t i=0;i<pba->cs2_num_super_sample_knots-1;i++){
          // Via the atanh(cs2) spline evaluate cs2 and d2cs2/dlog10a2 on interval ends
          // Use these to approximate cs2(log10a) as a cubic in log10a
          // For clarity we map log10a to t = (log10a - cs2_log10a_vals[i])/h
          // which linearly maps the original interval to [0,1]
          // this sets cs2_i(0)=cs21, cs2_i(1)=cs22,  cs2_i''(t)= h^2 * d2cs2_by_dlog10a2
          // d2cs2_by_dlog10a2 = (1-cs2^2)*(d2atanh_cs2_by_dlog10a2 - 2*cs2*datanh_cs2_by_dlog10a^2)
          h = pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2_log10a_super]
              - pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2_log10a_super];            // interval size
          cs21= pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2];  // w on left boundary
          cs22= pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2];  // w on right boundary
          ddcs21=h*h*(1-cs21*cs21)*(pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]
                             -2*cs21*pow(pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2_atanh]
                                      -pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2_atanh]
                                      -1/6.0*(2*pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]
                                              +pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]),2));   // w''(t) on left boundary
          ddcs22=h*h*(1-cs22*cs22)*(pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]
                            -2*cs22*pow(pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_cs2_atanh]
                                      -pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2_atanh]
                                      +1/6.0*(pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]
                                              +2*pba->cs2_array[(i+1)*pba->cs2_array_num_cols+pba->index_d2atanh_cs2_by_dlog10a2]),2)); // w''(t) on right boundary
          ...
          // Calculate int(cs2) on interval [cs2_log10a_vals[0],log10a_alpha]
          if(i ==  cs2_ref_interval_idx){
            t = (log10a_alpha- pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2_log10a_super])/h;
            r = 1-t;
            wrksp_intgrl=pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
                       +h*(t*t*(12.0*cs22+(t*t-2)*ddcs22)/24.0
                           -(r*r-1)*(12.0*cs21+(r*r-1)*ddcs21)/24.0);
          ...
          if(i ==  cs2_a_today_interval_idx){
            t = (log10a_today - pba->cs2_array[i*pba->cs2_array_num_cols+pba->index_cs2_log10a_super])/h;
            r = 1-t;
            // store the intergral from a_alpha to a_today, since a_alpha <= a_today the integral 0 shift will have already occured
            int_cs2_alpha_to_today=pba->cs2_array[cs2_a_today_interval_idx*pba->cs2_array_num_cols+pba->index_int_cs2_dlog10a]
                                 +h*(t*t*(12.0*cs22+(t*t-2)*ddcs22)/24.0-(r*r-1)*(12.0*cs21+(r*r-1)*ddcs21)/24.0);
...
int input_default_params(struct background *pba,
  ...
  //dsg cs2 reconstruction addition
  pba->recons_wfld = 0;
  pba->cs2_on = 0;
