...
/** dsg cs2 additons
    list of interpolation methods for cs2:  */

enum cs2_interpolation_method {cs2_linear, cs2_cubic, cs2_cubic_arctanh};
...
struct background
{
  ...
  double cs2_fld_0;  /**< \f$ c^2_{s~DE} \f$: sound speed of the fluid in the frame comoving with the fluid (so, this is
  ...
  int index_bg_cs2_fld;      /**< index for storage of the cs2 of the fluid */
  ...
  /** @name - wspl parameters */

  //@{

  short   has_wspl;             /**< presence of cs2 terms  */
  int     w_num_in_knots;    /**< the number knots of delta(log(a)) provided */
  double  *w_log10a_vals;    /**< an array of log10(a) values used as independent knots for w(log10(a)) */
  double  *w_array;        /**< an array of length (cs2_num_in_knots)*3 stores (cs2(log10(a)),cs2'(log10(a)),cs2''(log10(a)), int cs2 dlog10 a) */
  double  Omega0_wspl;       /**< \f$ \Omega_{0 de} \f$: wspl */

  double cs2_0;
  double cs2_1;
  int     index_w;               /**< the index where cs2(ln(a)) is stored in the cs2 array*/
  int     index_dw_by_dlog10a;           /**< the index where w'(log10(a)) is stored will be used for the spline  */
  int     index_int_w_dlog10a;   /**<  the index where integral cs2(ln(a)) dlog10(a) */
  int     w_array_num_cols;      /**< number of cols in cs2 array */

  int     w_last_index;         /**< keeps track of the last index used to speed up spline interpolations */

  double  w_z_alpha;         /** reference redshift for energy density , FIXED TO 0 IN INPUT.C*/

  int     index_bg_wspl;      /**< index for storage of the state parameter of the Generalized dark matter fluid */
  int     index_bg_dwspl_over_dlna;      /**< index for storage of the derivative of the equation of state parameter of the Generalized dark matter fluid */
  int     index_bg_int_wspl_dlna;      /**< index for storage of the derivative of the equation of state parameter of the Generalized dark matter fluid */
  int     index_bg_rho_wspl;      /**< index for storage of the state parameter of the Generalized dark matter fluid */
  int     index_bg_cs2_wspl;
  //@}

  
  /** @name - dsg cs2 parameters */

  //@{

  short   has_cs2;             /**< presence of cs2 terms  */
  short   cs2_interpolation_method;     /**< stores how to convert from the spline to w space (0 - spline (identity map), 1 - tanh(spl) space) */
  int     cs2_num_in_knots;    /**< the number knots of delta(log(a)) provided */
  double  *cs2_log10a_vals;    /**< an array of log10(a) values used as independent knots for w(log10(a)) */
  double  *cs2_array;        /**< an array of length (cs2_num_in_knots)*3 stores (cs2(log10(a)),cs2'(log10(a)),cs2''(log10(a)), int cs2 dlog10 a) */
  int     recons_wfld;  /** Linear spline reconstruction following eq of state cs2 * (abs(w) + 1.)  **/

  //tanh modulated spline
  int     index_cs2_log10a_super;          /**< the index where the super sampled log10a values for the spline are stored */
  int     index_cs2_atanh;               /**< the index where atanh(w(ln(a))) is stored in the cs2 atanh array*/
  int     index_d2atanh_cs2_by_dlog10a2; /**< the index where atanh(w''(log10(a))) is stored in the cs2 atanh array  */
  int     index_dcs2_by_dlog10a;           /**< the index where cs2'(log10(a)) is stored will be used for the spline  */
  int     cs2_num_super_sample_knots;      /**< the numer of knowts for the super sampled splined used for tanh spline */

  int     index_cs2;               /**< the index where cs2(ln(a)) is stored in the cs2 array*/
  int     index_d2cs2_by_dlog10a2; /**< the index where cs2''(log10(a)) is stored in the cs2  */
  int     index_int_cs2_dlog10a;   /**<  the index where integral cs2(ln(a)) dlog10(a) */
  int     cs2_array_num_cols;      /**< number of cols in cs2 array */

  int     cs2_last_index;         /**< keeps track of the last index used to speed up spline interpolations */

  double  cs2_z_alpha;         /** reference redshift for energy density */
  double  cs2_on;             /** turn on cs2 routine, 0 is turned off (default, set in input.c)**/

  int     index_bg_cs2;      /**< index for storage of the state parameter of the Generalized dark matter fluid */
  int     index_bg_dcs2_over_dlna;      /**< index for storage of the derivative of the equation of state parameter of the Generalized dark matter fluid */
  int     index_bg_int_cs2_dlna;      /**< index for storage of the derivative of the equation of state parameter of the Generalized dark matter fluid */
